...
def lambda_handler(event, context):
    ...
    if fps and fps > 0:
        # 2 frames per second → capture every (fps/2) frames
        frame_interval = max(1, int(fps / 2))
    else:
        frame_interval = 15  # fallback if fps missing
    ...
    cap.release()
    logger.info(f"Extracted {saved_count} frames at ~2 fps")
